...
static async Task AddCustomerInteractive()
{
    ...
    {
        Console.WriteLine("Customer email is required.");
        return;
    }
...
static async Task AddOrderInteractive()
{
    ...
    Console.WriteLine($"Order added: {order.OrderID} - Total: ${order.TotalAmount:0.00} on {order.OrderDate}:u for {customer.Name}");
}
...
static async Task ViewOrdersInteractive()
{
    ...
    foreach (var o in orders)
    {
        ...
        Console.WriteLine($" - {o.OrderID}: {o.OrderDate:u} | Total: ${o.TotalAmount:0.00} | Customer: {custName} (ID: {o.CustomerID})");
    }
}
